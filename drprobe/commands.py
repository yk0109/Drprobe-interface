...
def buildcell(output_file, spacegroup=None, lattice=None, atom=None, cif=False, output=False):
    _command = "BuildCell"
    if spacegroup is not None:
        _command += ' --spacegroup={}'.format(spacegroup)
    if lattice is not None:
        _command += ' --lattice={}'.format(lattice)
    if atom is not None:
        atom = np.atleast_1d(atom)
        for p in atom:
            _command += ' --atom={}'.format(p)
    _command += " --output={}".format(output_file)
# If necessary, create folder for output slices.
    directory = os.path.split(output_file)[0]
    if directory:
        if not os.path.isdir(directory):
            os.makedirs(directory, exist_ok=True)
# Run the BuildCell command
    if output:
...
def cellmuncher(cel_file, output_file, attach_cel=None, attach_direction=None,
                translate=None, periodic=None, repeat=None, set_dwf=None, frozen_lattice=None, remove_close_atoms=None,
                sort=None, cif=False, override=False, output=False, create_block=None, rotate_non_periodic=None,
                write_format=None, attach_cell=None,
                swap_axes=None):
    """
    ...
    remove_close_atoms : float or (float, str), optional
        ...
        in the list. Distance in Angstrom
. String: optional file-name to save an XMS cel file
    ...
    if translate is not None:
        translate = np.atleast_2d(translate)
        for r in translate:
            _command += ' -T {},{}'.format(r[0], r[1])
    if periodic is not None:
        periodic = np.atleast_1d(periodic)
        for p in periodic:
            _command += ' --periodic={}'.format(p[0])
    ...
    if create_block is not None:
        _command += ' --create-block={}'.format(create_block)
    if rotate_non_periodic is not None:
        _command += ' --rotate-non-periodic={}'.format(rotate_non_periodic)
    if write_format is not None:
        _command += ' --write-format={}'.format(write_format)
    if attach_cell is not None:
        _command += ' --attach-cell={}'.format(attach_cell)
    if swap_axes is not None:
        _command += ' --swap-axes={}'.format(swap_axes)
