...
class MsaPrm(object):
    ...
    def __init__(self, msa_dict=None, aberrations_dict=None):
        ...
        self.conv_semi_angle = msa_dict.get('conv_semi_angle', 25.0)
        self.inner_radius_ann_det = msa_dict.get('inner_radius_ann_det', 0.0)
        self.outer_radius_ann_det = msa_dict.get('outer_radius_ann_det', 20.0)
        self.detector = msa_dict.get('detector', (1, "prm/detectors.prm"))
        self.wavelength = msa_dict.get('wavelength', 200.0)
        self.source_radius = msa_dict.get('source_radius', 0.01)
        self.focus_spread = msa_dict.get('focus_spread', 3.00)
        self.focus_spread_kernel_hw = msa_dict.get('focus_spread_kernel_hw', 2.0)
        self.focus_spread_kernel_size = msa_dict.get('focus_spread_kernel_size', 7)
        # self.number_of_aber = msa_dict.get('number_of_aber', 0)
        self.tilt_x = msa_dict.get('tilt_x', 0.000000)
        self.tilt_y = msa_dict.get('tilt_y', 0.000000)
        self.h_scan_offset = msa_dict.get('h_scan_offset', 0.0)
        self.v_scan_offset = msa_dict.get('v_scan_offset', 0.0)
        ...
        self.scan_columns = msa_dict.get('scan_columns', 1)
        self.scan_rows = msa_dict.get('scan_rows', 1)
    ...
    def save_msa_prm(self, prm_filename, random_slices=False, output=False):
        ...
        with open(prm_filename, 'w') as prm:
            ...
            string_0 = "[mrad] illumination cone half-angle     ! STEM only"
            if isinstance(self.conv_semi_angle, (int, float)):
                ...
                prm.write("{} ! {}\n".format(0, string_0))
            string_1 = "[mrad] inner detection angle            ! STEM only"
            prm.write("{} ! {}\n".format(self.inner_radius_ann_det, string_1))
            string_2 = "[mrad] outer detection angle            ! STEM only"
            prm.write("{} ! {}\n".format(self.outer_radius_ann_det, string_2))
            string_3 = "flag and name of STEM detector file     ! STEM only"
            prm.write("{}, '{}'! {}\n".format(self.detector[0], self.detector[1], string_3))
            string_4 = "[keV] electron energy"
            prm.write("{} ! {}\n".format(self.wavelength, string_4))
            string_5 = "[nm] geometrical source size (HWHM)     ! STEM only"
            prm.write("{} ! {}\n".format(self.source_radius, string_5))
            string_6 = "[nm] Focus spread                       ! STEM only"
            prm.write("{} ! {}\n".format(self.focus_spread, string_6))
            string_7 = "relative focus spread kernel width      ! STEM only"
            prm.write("{} ! {}\n".format(self.focus_spread_kernel_hw, string_7))
            string_8 = "focus spread kernel samples             ! STEM only"
            prm.write("{} ! {}\n".format(self.focus_spread_kernel_size, string_8))
            string_9 = "number of probe aberrations             ! STEM only"
            ...
            string_10 = "[deg] object tilt X, stay below 5 deg!"
            prm.write("{} ! {}\n".format(self.tilt_x, string_10))
            string_11 = "[deg] object tilt Y, stay below 5 deg!"
            prm.write("{} ! {}\n".format(self.tilt_y, string_11))
            string_12 = "[nm] horizontal scan frame offset       ! STEM only"
            prm.write("{} ! {}\n".format(self.h_scan_offset, string_12))
            string_13 = "[nm] vertical scan frame offset         ! STEM only"
            prm.write("{} ! {}\n".format(self.v_scan_offset, string_13))
            string_14 = "[nm] horizontal scan frame size         ! STEM only"
            prm.write("{} ! {}\n".format(self.h_scan_frame_size, string_14))
            string_15 = "[nm] vertical scan frame size           ! STEM only"
            prm.write("{} ! {}\n".format(self.v_scan_frame_size, string_15))
            string_16 = "[deg] scan frame rotation               ! STEM only"
            prm.write("{} ! {}\n".format(self.scan_frame_rot, string_16))
            string_17 = "horizontal scan frame samples           ! STEM only"
            prm.write("{} ! {}\n".format(self.scan_columns, string_17))
            string_18 = "vertical scan frame samples             ! STEM only"
            prm.write("{} ! {}\n".format(self.scan_rows, string_18))
            string_19 = "switch for partial temporal coherence   ! STEM only"
            prm.write("{} ! {}\n".format(self.temp_coherence_flag, string_19))
            string_20 = "switch for partial spatial coherence    ! STEM only"
            prm.write("{} ! {}\n".format(self.spat_coherence_flag, string_20))
            string_21 = "super-cell X repeat"
            prm.write("{} ! {}\n".format(self.super_cell_x, string_21))
            string_22 = "super-cell Y repeat"
            prm.write("{} ! {}\n".format(self.super_cell_y, string_22))
            string_23 = "super-cell Z repeat factor              ! Ignored "
            prm.write("{} ! {}\n".format(self.super_cell_z, string_23))
            string_24 = "phase grating files (without indices and extension)"
            ...
            string_29 = "max. Number of slices in the object."
